...
CursoR <- c("blogdown, hugo-themes, Rmarkdown")

TS <- c("Detecção de Mudanças","Análise de tendências e anomalias")




classes <- tibble(title =c(rep("\\href{https://viniciovcl.github.io/viniciovcl/post/}{\\color{black}{Webpages (link)}}", length(CursoR)),
                            rep("\\href{https://github.com/viniciovcl/Data-cube-raster/}{\\color{black}{Séries Temporais de Imagens de Satélite (link)}}",length(TS))
                            ),
                  detail = c(CursoR,  TS))
...
tribble(
   ~`Linguagem`, ~`GIS`,  ~`Documentação e comunicação`,
  "R, SQL",
  "QGIS, ArcGIS, GDAL, GEOS, SAGA, LidR, sf, terra, dplyr, Earth-Engine",
  "r-markdown, markdown, \\LaTeX, Git, ggplot, Flexdashboard, Shiny, tmap, leafleat, mapview"
) %>%
...
structure(
  list(
    ...
    why = list(
      ...
      "Bacharelado em Geografia | Área de Concentração: Geografia Física"
    )
  ),
  row.names = c(NA, 3L),
  groups = structure(
